...
public class MenuSelectAction extends Action {
	public int doBusiness() {
          ...
          String SQLStr = "select  menuid,menulabel,menudesc,(select  count(menuid) as a from ptmenu   where (isleaf=0) and parentmenuid=me.menuid) as menucount ,Levelidx  from ptmenu  me where (isleaf=0) and parentmenuid='" +
                      this.req.getFieldValue("parentid") + "' order by Levelidx";

          this.rs = this.dc.executeQuery(SQLStr);

		this.res.setFieldName("menuid;menulabel;menudesc;menucount;Levelidx");
