...
try{
  do{
    ...
    response.addHeader("Content-Disposition", "attachment; filename=" + CcbLoanConst.RPT_ODSB_CHECK_PROJ+".xls");
    // ----------------------------根据模板创建新workbook得到可操作的writableworkbook写到输出流中-----------------------------------
    //得到报表模板
    String rptModelPath = PropertyManager.getProperty("REPORT_ROOTPATH");
    String rptName = rptModelPath + CcbLoanConst.RPT_ODSB_CHECK_PROJ+".xls";   
    ...
    RecordSet rs=conn.executeQuery(""
       +" select (select deptname from ptdept where deptid=a.bankid)bankid,"
       +" a.LOANID,a.cust_name,a.proj_no  from ln_odsb_loanapply a where not" 
       +" exists(select 1 from ln_coopproj b where b.proj_no=a.proj_no)"
       +" and trim(a.proj_no) is not null " );
    // 行计数器
    int i = 0;
    // 步长（每条记录输出1行）
    int step = 1;
    ...
    while(rs.next()){
      ...
      if(i > 0){
        cnt = cnt -i;
   	  ...
   	  // 合作方编号	  
   	  Label lbl = new Label(beginCol,i+beginRow+cnt,rs.getString("bankid"),wcf_center);
      ws.addCell(lbl);    
      lbl = new Label(beginCol+1,i+beginRow+cnt,rs.getString("loanid"),wcf_center);
      ws.addCell(lbl);    
      lbl = new Label(beginCol+2,i+beginRow+cnt,rs.getString("cust_name"),wcf_center);
      ws.addCell(lbl);
      lbl = new Label(beginCol+3,i+beginRow+cnt,rs.getString("proj_no"),wcf_center);
      ws.addCell(lbl);    
      i++;
    }  
