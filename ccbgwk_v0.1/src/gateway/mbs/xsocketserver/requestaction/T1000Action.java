...
//        List rtnlist = queryConsumeInfo(voucherid, year);
        List rtnlist = queryConsumeInfoTest(voucherid, year);
    ...
    private void generateResponseData(List<LSPAYBACKINFO> paybackInfos) {
        ...
        if (isMultiResponsePkg) {
            ...
            responseDataList.add(generateOneReponseDataPkg(true, body));
            ...
            for (int i = 0; i < loopcount; i++) {
                ...
                responseDataList.add(generateOneReponseDataPkg(false, body));
                count += this.recordNum;
            }

            //最后一个包
            body = getResponseDataBodyContent(paybackInfos, count, listsize);
            ResponseData responseData = generateOneReponseDataPkg(false, body);
            //设置为无后续包标志 !!
    ...
    */
     ...
     * @return
     */
    private ResponseData generateOneReponseDataPkg(boolean isFirstPkg, String bodyStr) {
        ...
        if (isFirstPkg) {
            ...
            //包头长度处理
            length = this.headLength + this.bodyHeadFieldlength + this.oneRecordLength * this.totalCount;
        } else {
            responseData.setBodyData(bodyStr);
            //包头长度处理
            length = this.headLength + this.oneRecordLength * this.totalCount;
        }
