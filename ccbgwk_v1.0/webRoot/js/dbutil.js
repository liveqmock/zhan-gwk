...
function postExcute(XMLStr) {
  var retStr = ExecServerPrgm("/BI/util/SqlExcuteJsp.jsp", "POST", "sys_request_xml=" + XMLStr);
  return retStr;
}
...
function createExecuteform(formelement, exctype, actionName, methodname) {
  ...
  if (true) {
    ...
    // retStr =
    // ExecServerPrgm_synsh("/BI/util/SqlExcuteJsp.jsp","POST","sys_request_xml="+xmlDoc.xml,"","","",fun_test);
...
function createExecuteform_synsh(formelement, exctype, actionName, methodname) {
  ...
  if (checkForm(formelement) == "true") {
    ...
    if (xmlDoc.documentElement.childNodes.length > 0)
      ...
      // return;
      // retStr =
      // ExecServerPrgm("/BI/util/SqlExcuteJsp.jsp","POST","sys_request_xml="+xmlDoc.xml);

      retStr = ExecServerPrgm_synsh("/BI/util/SqlExcuteJsp.jsp", "POST", "sys_request_xml=" + xmlDoc.xml, "", "", "",
          fun_test);
...
function showErrorPage(errorMsg) {
  ...
  var spath = "/UI/system/public/errorShow.jsp";
  var arg = new Object();
