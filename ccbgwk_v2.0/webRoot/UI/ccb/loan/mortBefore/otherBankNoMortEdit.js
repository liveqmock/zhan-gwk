...
function formInit() {
	...
	if (arg) {
		...
		// 快递日期自动反显
		var date = new Date();
		document.getElementById("RPTNOMORTDATE").value = getDateString(date);
	...
	// 设置默认焦点；抵押交接标志
	if (operation != "select") {
		document.getElementById("RPTNOMORTDATE").focus();
	}
}
...
function saveClick() {
	...
	if (operation == "add") {
		...
		document.getElementById("busiNode").value = BUSINODE_030;
