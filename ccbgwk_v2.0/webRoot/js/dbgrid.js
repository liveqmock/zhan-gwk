...
function Table_Refresh(id, ishint, bodyReload, empty_title, login) {
    ...
    var tmpHTML = ExecServerPrgm_synsh("/BI/util/DataTableJsp.jsp", "POST", "tabStr=" + encode(xmlDoc.xml), "", "", id,
            Table_Refresh_Date);
...
function Table_Refresh_asy(id) {
    ...
    var tmpHTML = ExecServerPrgm("/BI/util/DataTableJsp.jsp", "POST", "tabStr=" + encode(xmlDoc.xml));
...
function printLExcel(printtype, tab) {
    ...
    if (printtype == "0") {
        ...
        // alert(xmlDoc.xml);
        // if( tab.RecordCount/1<200){
        // ExecServerPrgm_synsh("/BI/util/DropDownJsp.jsp","POST","xx="+encode(xmlDoc.xml),"","",tab.id,Refresh_Excel);
        // }else

        // ExecServerPrgm_synsh("/BI/util/expExcelJsp.jsp","POST","xx="+encode(xmlDoc.xml),"","",tab.id,open_Excel);

        openWindowWithPost("/BI/util/expExcelJsp.jsp", "Excel", encode(xmlDoc.xml));
        return;
    }
