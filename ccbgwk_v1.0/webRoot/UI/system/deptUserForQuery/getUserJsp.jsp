...
<title>
	...
	<LINK href="/css/catv.css" type="text/css" rel="stylesheet">
	...
	function onUserDocumentLoad()
     {
          ...
          if (retStr != false)
          {
			...
			if(isdept =="1")
               	var nRoot = vNodes.add("", "", "0", "CATV部门列表", "/images/domain.gif");
               else
               	var nRoot = vNodes.add("", "", "0", "CATV人员列表", "/images/domain.gif");
     ...
     function showApplications(eleRoot, nFather, bManagedApp)
     {
          ...
          while(node)
          {
               ...
               if(decode(getAttrValue(node.childNodes[2],"value"))=="0")
                    var nApp = nFather.add("tvwChild", decode(getAttrValue(node.childNodes[0],"value")), decode(getAttrValue(node.childNodes[1],"value"))+"("+decode(getAttrValue(node.childNodes[0],"value"))+")", "/images/about.gif");
               else
                    var nApp = nFather.add("tvwChild", decode(getAttrValue(node.childNodes[0],"value")), decode(getAttrValue(node.childNodes[1],"value"))+"("+decode(getAttrValue(node.childNodes[0],"value"))+")", "/images/user.gif");
          ...
          try{
               ...
               if (retStr != "false")
               {
                    ...
                    while(node)
                    {
                         ...
                         if(decode(getAttrValue(node.childNodes[2],"value"))=="0")
                              var nApp = n.add("tvwChild", decode(getAttrValue(node.childNodes[0],"value")), decode(getAttrValue(node.childNodes[1],"value"))+"("+decode(getAttrValue(node.childNodes[0],"value"))+")", "/images/about.gif");
                         else
                              var nApp = n.add("tvwChild", decode(getAttrValue(node.childNodes[0],"value")), decode(getAttrValue(node.childNodes[1],"value"))+"("+decode(getAttrValue(node.childNodes[0],"value"))+")", "/images/user.gif");
	...
	<table style="WIDTH:100%;HEIGHT:100%;left:0px" cellspacing="0" ID="Table1">
              <tr style="HEIGHT:64px">
				<td class="borderGrooveB">
					<font face="SimSun" style="FONT-WEIGHT: bold; FONT-SIZE: 18pt"><img class="shadowAlpha" align="absMiddle" src="/images/domain.gif" width="48" height="48">人员列表</font>
				</td>
			</tr>
