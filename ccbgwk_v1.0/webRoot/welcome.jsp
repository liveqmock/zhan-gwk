...
//    String uaapuser = request.getHeader("uid");
    String uaapuser;
    uaapuser = (String)session.getAttribute("uaapuser");
    if (uaapuser !=null && uaapuser !="") {
//        session.setAttribute("uaapuser",uaapuser);
        session.removeAttribute("uaapuser");
        session.invalidate();
        response.sendRedirect(basePath+"qdgddy");
//        response.sendRedirect(basePath+"pages/frame/homePage.jsp");
//        response.sendRedirect(basePath+"index.jsp");
    }else{
        uaapuser = request.getHeader("uid");
        if (uaapuser !=null && uaapuser !="") {
            session.setAttribute("uaapuser",uaapuser);
            response.sendRedirect(basePath+"pages/frame/homePage.jsp");
        }else{
